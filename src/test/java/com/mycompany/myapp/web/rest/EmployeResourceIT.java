...
class EmployeResourceIT {
    ...
    public static Employe createEntity(EntityManager em) {
        ...
        // Add required entity
        IService iService;
        if (TestUtil.findAll(em, IService.class).isEmpty()) {
            iService = IServiceResourceIT.createEntity(em);
            em.persist(iService);
            em.flush();
        } else {
            iService = TestUtil.findAll(em, IService.class).get(0);
        }
        employe.setService(iService);
        return employe;
    }
    ...
    public static Employe createUpdatedEntity(EntityManager em) {
        ...
        IService iService;
        if (TestUtil.findAll(em, IService.class).isEmpty()) {
            iService = IServiceResourceIT.createUpdatedEntity(em);
            em.persist(iService);
            em.flush();
        } else {
            iService = TestUtil.findAll(em, IService.class).get(0);
